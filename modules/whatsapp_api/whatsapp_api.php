 ...
 * 
 * تم تغيير القوائم من add_sidebar_menu_item إلى add_setup_menu_item
 * 
 *  * تم تغيير القوائم من add_sidebar_children_item إلى add_setup_children_item
 * 
*/
...
function whatsapp_api_module_init_menu_items()
{
    ...
    if (
        ...
        $CI->app_menu->add_setup_menu_item('whatsapp_api', [ //$CI->app_menu->add_sidebar_menu_item('whatsapp_api', [  تم تغييرها من اللمسة النهائية
            'slug'     => 'whatsapp_api',
            'name'     => _l('whatsapp'),
            'position' => 30,
            //'icon'     => 'fa fa-brands fa-whatsapp menu-icon', // تم تغييرها من اللمسة النهائية
        ]);

        if (has_permission('whatsapp_api', '', 'list_templates_view')) {
            $CI->app_menu->add_setup_children_item('whatsapp_api', [ //$CI->app_menu->add_sidebar_children_item('whatsapp_api', [ // تم تغييرها من اللمسة النهائية
