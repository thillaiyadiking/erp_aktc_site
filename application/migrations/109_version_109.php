...
class Migration_Version_109 extends CI_Migration
{
  ...
  public function up()
  {
    ...
    $this->db->query("INSERT INTO `tblemailtemplates` (`type`, `slug`, `name`, `subject`, `message`, `fromname`, `fromemail`, `plaintext`, `active`, `order`) VALUES
      ('tasks', 'task-deadline-notification', 'Task Deadline Reminder - Sent to assignees', 'Task Deadline Reminder', '<div>Hi {staff_firstname}</div><div><br></div><div>This is an automated email from <span style=\"font-family: Tajawal, Helvetica, Arial, sans-serif;\">{companyname}.</span></div><div>The task <span style=\"font-family: Tajawal, Helvetica, Arial, sans-serif;\">{task_name} deadline is on {task_duedate}. This task is still not finished.</span></div><div><br></div><div><span style=\"font-family: Tajawal, Helvetica, Arial, sans-serif;\">Please click on the following link to view: </span><span style=\"font-family: Tajawal, Helvetica, Arial, sans-serif; background-color: inherit;\">{task_link}</span></div><div><br></div><div>{email_signature}</div>', '{companyname} | ERP', '', 0, 1, 0)");
