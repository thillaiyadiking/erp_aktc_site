...
class CronJobFailure extends AbstractMessage
{
    ...
    public function getMessage()
    {
        ...
        if (file_exists(get_temp_dir() . 'perp-cron-lock')) {
            @unlink(get_temp_dir() . 'perp-cron-lock');
        }

        if (file_exists(TEMP_FOLDER . 'perp-cron-lock')) {
            @unlink(TEMP_FOLDER . 'perp-cron-lock');
        } ?>
