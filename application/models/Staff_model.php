...
class Staff_model extends App_Model
{
    ...
    public function add($data)
    {
        ...
        $stf_br_assgn = $data['branch_assignment'];
        unset($data['branch_assignment']);

        $this->db->insert(db_prefix() . 'staff', $data);
        $staffid = $this->db->insert_id();
        
        if(isset($stf_br_assgn)){
            $this->add_staff_branches($staffid,$stf_br_assgn);
    ...
    public function update($data, $id)
    {
        ...
        // }
        $stf_br_assgn = [];
        if (isset($data['branch_assignment'])) {
            $stf_br_assgn = $data['branch_assignment'];
            unset($data['branch_assignment']);
        }
        ...
        if(isset($stf_br_assgn)){
            $this->db->where('staffid', $id);
            $this->db->delete(db_prefix() . 'staff_branch_assignment');
            $this->add_staff_branches($id,$stf_br_assgn);
    ...
    public function add_staff_branches($stf,$stf_branches=[]){

        if(sizeof($stf_branches)>0){
            foreach($stf_branches as $br_assgn){
                $this->db->insert(db_prefix() . 'staff_branch_assignment', [
                    'staffid'      => $stf,
                    'branch_id' => $br_assgn,
                ]);
            }
    ...
    public function get_assigned_branches($id){
        $staff_branches=[];
        $this->db->select('branch_id');        
        $this->db->where('staffid', $id);
        $staff_branches = $this->db->get(db_prefix() . 'staff_branch_assignment')->result();
        
        return $staff_branches;
    }
}
