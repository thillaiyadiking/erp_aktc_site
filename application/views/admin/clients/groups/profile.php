            ...
            <div role="tabpanel" class="tab-pane<?php if (!$this->input->get('tab')) {
                ...
                <div class="row">
                    ...
                    <div class="col-md-<?php echo !isset($client) ? 12 : 8; ?>">
                        ...
                        <?php
                        // echo $edit_data_id;
                        // echo $client->is_supplier;
                        $custom_fields =
                            [
                                'id' => $edit_data_id ? $client->userid : "",
                                'label_name' => '',
                                'name' => 'is_supplier',
                                'type' => 'checkbox',
                                'is_supplier' => $edit_data_id ? $client->is_supplier : "",
                                'required' => 1,
                                'options' => 'is_supplier',
                                'display_inline' => 1, // Determines if checkboxes are displayed inline
                            ]
                            // other custom fields...
                        ;
                        echo (generate_html_checkboxes($custom_fields));
                        ?>
                        ...
                        if (get_option('company_requires_vat_number_field') == 1) {
                            $value = (isset($client) ? $client->commercial_reg_num : '');
                            echo render_input('commercial_reg_num', 'commercial_reg_num', $value);
                        } ?>
                        ...
                        <?php $value = (isset($client) ? $client->address : $branch_det->branch_address); ?>
                        <?php echo render_textarea('address', 'client_address', $value); ?>
                        <?php $value = (isset($client) ? $client->city : $branch_det->branch_city); ?>
                        <?php echo render_input('city', 'client_city', $value); ?>
                        <?php $value = (isset($client) ? $client->state : $branch_det->branch_state); ?>
                        <?php echo render_input('state', 'client_state', $value); ?>
                        <?php $value = (isset($client) ? $client->zip : $branch_det->branch_postal_code); ?>
                        <?php echo render_input('zip', 'client_postal_code', $value); ?>
                        <?php $countries       = get_all_countries();
                        if (isset($client)) {
                            $selected = $client->country;
                        } elseif (get_option('customer_default_country') != '') {
                            $selected = get_option('customer_default_country');
                        } else {
                            $selected = $branch_det->branch_country;
                        }
...
<script>
    ...
    function removeField(button) {
        button.closest('.bank-group').remove();
    }

    function attachIbanPrefix() {
        document.querySelectorAll('.iban-input').forEach(input => {
            if (!input.dataset.initialized) {
                input.value = input.value.startsWith('SA') ? input.value : 'SA';
