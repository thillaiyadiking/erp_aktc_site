...
$aColumns = [
    ...
    'file_name',
    'date',
    '(SELECT concat(firstname, \' \',lastname) FROM ' . db_prefix() . 'staff WHERE ' . db_prefix() . 'staff.staffid = ' . db_prefix() . 'expenses.purchased_staff) as purchased_staff',
    '(SELECT concat(firstname, \' \',lastname) FROM ' . db_prefix() . 'staff WHERE ' . db_prefix() . 'staff.staffid = ' . db_prefix() . 'expenses.recipient_staff) as recipient_staff',    
...
foreach ($rResult as $aRow) {
    ...
    $row[] = $aRow['purchased_staff'];

    $row[] = $aRow['recipient_staff'];
