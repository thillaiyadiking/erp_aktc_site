            ...
            <div class="col-md-6">
                ...
                <div class="panel_s">
                    <div class="panel-body">
                        ...
                        <?php
                        $purchased = (isset($expense) ? $expense->purchased_staff : '');

                        if (!empty($last_inserted_expense) && !empty($last_inserted_expense->purchased_staff)) {
                            $purchased = $last_inserted_expense->purchased_staff;
                        }

                        echo render_select('purchased_staff', $members, ['staffid', ['firstname', 'lastname']], _l('Purchased_Employee'), $purchased);

                        ?>

                        <?php

                        if (!isset($expense) || (isset($expense) && $expense->recipient_staff != 0)) {
                            $recipient = (isset($expense) ? $expense->recipient_staff : '');


                            // if (!empty($last_inserted_expense) && !empty($last_inserted_expense->recipient_staff)) {
                            //     $recipient   = $last_inserted_expense->recipient_staff;
                            // }


                            echo render_select('recipient_staff', $members, ['staffid', ['firstname', 'lastname']], _l('Recipient_Employee'), $recipient);
                        }
                        ...
                        if (!isset($expense) || (isset($expense) && $expense->clientid != 0)) { ?>
                        ...
                        if ((isset($expense) && $expense->clientid != 0) || isset($customer_id) || (isset($expense) && $expense->recipient_staff != 0)) {
                        ...
                        <?php
                        if (!isset($expense) || (isset($expense))) {
                            $supplier = (isset($expense) ? $expense->supplier_id : '');

                            if (!empty($last_inserted_expense) && !empty($last_inserted_expense->supplier_id)) {
                                $supplier = $last_inserted_expense->supplier_id;
                            }

                            echo render_select('supplier_id', $clients, ['userid', ['company']], _l('supplier'), $supplier);
                        }
                        ?>
                        ...
                        <?php
                        $value = (isset($expense) ? $expense->expense_name : '');
                        ?>
                        <?php echo render_input('expense_name', 'expense_name', $value, '', ['disabled' => true]); ?>
                        <i class="fa-regular fa-circle-question pull-left tw-mt-0.5 tw-mr-1" data-toggle="tooltip"
                            data-title="<?php echo _l('expense_field_billable_help', _l('expense_add_edit_note')); ?>"></i>
                        <?php
                        $value = (isset($expense) ? $expense->note : '');
                        if (!empty($last_inserted_expense) && !empty($last_inserted_expense->note)) {
                            $value = $last_inserted_expense->note;
                        }
                        ?>
                        <?php echo render_textarea('note', 'expense_add_edit_note', $value, ['rows' => 4], []); ?>
                        <?php
                        $selected = (isset($expense) ? $expense->category : '');
                        if (!empty($last_inserted_expense) && !empty($last_inserted_expense->category)) {
                            $selected = $last_inserted_expense->category;
                        }
                        ...
                        if (!empty($last_inserted_expense) && !empty($last_inserted_expense->date)) {
                            $value = $last_inserted_expense->date;
                        }
                        $date_attrs     = [];
                        ...
                        ?>
                        <?php
                        echo render_date_input('date', 'expense_add_edit_date', $value, $date_attrs);
                        $value = (isset($expense) ? $expense->amount : '');
                        // if (!empty($last_inserted_expense) && !empty($last_inserted_expense->amount)) {
                        //     $value = $last_inserted_expense->amount;
                        // }
                        ?>
                ...
                <div class="panel_s">
                    <div class="panel-body">
                                ...
                                <div class="form-group select-placeholder">
                                    ...
                                    <select class="selectpicker display-block" data-width="100%" name="tax"
                                        ...
                                        <?php foreach ($taxes as $tax) {
                                            ...
                                            if (!empty($last_inserted_expense) && !empty($last_inserted_expense->tax)) {
                                                // $value = $last_inserted_expense->amount;
                                                $selected = 'selected';
                                            }
                        ...
                        <div class="row">
                            <div class="col-md-6">
                                <?php
                                $selected = (isset($expense) ? $expense->paymentmode : '');
                                if (!empty($last_inserted_expense) && !empty($last_inserted_expense->paymentmode)) {
                                    $selected = $last_inserted_expense->paymentmode;
                                }
                            ...
                            <div class="col-md-6">
                                <?php 
                                $value = (isset($expense) ? $expense->reference_no : '');
                                if (!empty($last_inserted_expense) && !empty($last_inserted_expense->reference_no)) {
                                    $value = $last_inserted_expense->reference_no;
                                }
                                 ?>
...
<script>
    ...
    $(function() {
        ...
        $('#recipient_staff').on('change', function() {
            var projectsWrapper = $('.projects-wrapper');
            if ($(this).val() != '') {
                $('#clientid').parent('div').parent('div').hide();
                $('.projects-wrapper').removeClass('hide');
            } else {
                $('#clientid').parent('div').parent('div').show();
        ...
        $('#clientid').on('change', function() {
            ...
            if ($(this).val() != '') {
                $('#recipient_staff').parent('div').parent('div').hide();
            } else {
                $('#recipient_staff').parent('div').parent('div').show();
            }
