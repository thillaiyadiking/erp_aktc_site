...
if (file_exists($imagePath)) {
    $imgWidth = 40; // Width of the image
    $imgHeight = 40; // Height of the image

    // Get page width minus right margin and image width
    $pageWidth = $pdf->getPageWidth();
    $rightMargin = $pdf->getMargins()['right'];
    $x = $pageWidth - $rightMargin - $imgWidth;

    // Get current Y position
    $y = $pdf->GetY();

    // Insert the image at calculated position
    $pdf->Image($imagePath, $x, $y, $imgWidth, $imgHeight, 'PNG');

    // Optional: move cursor down after image
    $pdf->Ln($imgHeight + 2);
}
