...
ERROR - 2023-09-27 14:51:58 --> Query error: You have an error in your SQL syntax; check the manual that corresponds to your MariaDB server version for the right syntax to use near 'OR branch_id=0 AND ( (tbltasks.id IN (SELECT taskid FROM tbltask_assigned WHE...' at line 17 - Invalid query: 
...
ERROR - 2023-09-27 14:57:13 --> Query error: You have an error in your SQL syntax; check the manual that corresponds to your MariaDB server version for the right syntax to use near 'OR branch_id=0 AND ( (tbltasks.id IN (SELECT taskid FROM tbltask_assigned WHE...' at line 17 - Invalid query: 
    SELECT SQL_CALC_FOUND_ROWS 1, tbltasks.id as id, tbltasks.name as task_name, status, startdate, duedate, (SELECT GROUP_CONCAT(CONCAT(firstname, ' ', lastname) ORDER BY tbltask_assigned.id ASC SEPARATOR ",") FROM tbltask_assigned JOIN tblstaff ON tblstaff.staffid = tbltask_assigned.staffid WHERE taskid=tbltasks.id) as assignees, (SELECT GROUP_CONCAT(name SEPARATOR ",") FROM tbltaggables JOIN tbltags ON tbltaggables.tag_id = tbltags.id WHERE rel_id = tbltasks.id and rel_type="task" ORDER by tag_order ASC) as tags, priority ,rel_type,rel_id,recurring,(CASE rel_type
        WHEN "contract" THEN (SELECT subject FROM tblcontracts WHERE tblcontracts.id = tbltasks.rel_id)
        WHEN "estimate" THEN (SELECT id FROM tblestimates WHERE tblestimates.id = tbltasks.rel_id)
        WHEN "proposal" THEN (SELECT id FROM tblproposals WHERE tblproposals.id = tbltasks.rel_id)
...
ERROR - 2023-09-27 15:58:50 --> Query error: Column 'branch_id' in where clause is ambiguous - Invalid query: 
    SELECT SQL_CALC_FOUND_ROWS tblinvoicepaymentrecords.id as id, invoiceid, paymentmode, transactionid, CASE tblclients.company WHEN ' ' THEN (SELECT CONCAT(firstname, ' ', lastname) FROM tblcontacts WHERE userid = tblclients.userid and is_primary = 1) ELSE tblclients.company END as company, amount, tblinvoicepaymentrecords.date as date ,clientid,tblcurrencies.name as currency_name,tblpayment_modes.name as payment_mode_name,tblpayment_modes.id as paymentmodeid,paymentmethod
    FROM tblinvoicepaymentrecords
    LEFT JOIN tblinvoices ON tblinvoices.id = tblinvoicepaymentrecords.invoiceid LEFT JOIN tblclients ON tblclients.userid = tblinvoices.clientid LEFT JOIN tblcurrencies ON tblcurrencies.id = tblinvoices.currency LEFT JOIN tblpayment_modes ON tblpayment_modes.id = tblinvoicepaymentrecords.paymentmode
    
    WHERE  branch_id=2
    
    ORDER BY tblinvoicepaymentrecords.id DESC
    LIMIT 0, 25
    
ERROR - 2023-09-27 15:59:08 --> Query error: Column 'branch_id' in where clause is ambiguous - Invalid query: 
    SELECT SQL_CALC_FOUND_ROWS tblinvoicepaymentrecords.id as id, invoiceid, paymentmode, transactionid, CASE tblclients.company WHEN ' ' THEN (SELECT CONCAT(firstname, ' ', lastname) FROM tblcontacts WHERE userid = tblclients.userid and is_primary = 1) ELSE tblclients.company END as company, amount, tblinvoicepaymentrecords.date as date ,clientid,tblcurrencies.name as currency_name,tblpayment_modes.name as payment_mode_name,tblpayment_modes.id as paymentmodeid,paymentmethod
    FROM tblinvoicepaymentrecords
    LEFT JOIN tblinvoices ON tblinvoices.id = tblinvoicepaymentrecords.invoiceid LEFT JOIN tblclients ON tblclients.userid = tblinvoices.clientid LEFT JOIN tblcurrencies ON tblcurrencies.id = tblinvoices.currency LEFT JOIN tblpayment_modes ON tblpayment_modes.id = tblinvoicepaymentrecords.paymentmode
...
ERROR - 2023-09-27 16:00:12 --> Query error: Column 'branch_id' in where clause is ambiguous - Invalid query: 
    SELECT SQL_CALC_FOUND_ROWS tblinvoicepaymentrecords.id as id, invoiceid, paymentmode, transactionid, CASE tblclients.company WHEN ' ' THEN (SELECT CONCAT(firstname, ' ', lastname) FROM tblcontacts WHERE userid = tblclients.userid and is_primary = 1) ELSE tblclients.company END as company, amount, tblinvoicepaymentrecords.date as date ,clientid,tblcurrencies.name as currency_name,tblpayment_modes.name as payment_mode_name,tblpayment_modes.id as paymentmodeid,paymentmethod
    FROM tblinvoicepaymentrecords
    LEFT JOIN tblinvoices ON tblinvoices.id = tblinvoicepaymentrecords.invoiceid LEFT JOIN tblclients ON tblclients.userid = tblinvoices.clientid LEFT JOIN tblcurrencies ON tblcurrencies.id = tblinvoices.currency LEFT JOIN tblpayment_modes ON tblpayment_modes.id = tblinvoicepaymentrecords.paymentmode
...
ERROR - 2023-09-27 16:04:37 --> Severity: error --> Exception: syntax error, unexpected 'static' (T_STATIC) C:\xampp\htdocs\finishizer_erp3.0.5\application\services\estimates\EstimatesPipeline.php 29
ERROR - 2023-09-27 16:28:20 --> Severity: error --> Exception: syntax error, unexpected 'static' (T_STATIC) C:\xampp\htdocs\finishizer_erp3.0.5\application\services\tasks\TasksKanban.php 39
ERROR - 2023-09-27 16:29:19 --> Severity: error --> Exception: syntax error, unexpected 'static' (T_STATIC) C:\xampp\htdocs\finishizer_erp3.0.5\application\services\tasks\TasksKanban.php 39
