...
class Clients extends AdminController
{
    ...
    public function client($id = '')
    {
        ...
        if ($this->input->post() && !$this->input->is_ajax_request()) {
            ...
            $bankDetailsJson = json_encode($bankDetails, JSON_UNESCAPED_UNICODE);

            // Now assign it to your new column
            $data['bank_details'] = $bankDetailsJson;
    ...
    public function form_contact($customer_id, $contact_id = '')
    {
        ...
        if ($this->input->post()) {
            ...
            if ($contact_id == '') {
                ...
                // nasik
                if ($success == true) {
                    set_alert('danger', _l('updated_successfully', _l('client')));
                }
            ...
            if ($success == true) {
                set_alert('danger', _l('failed_to_update', _l('client')));
            } else {
                set_alert('success', _l('updated_successfully', _l('client')));
            }
            redirect(admin_url('clients/all_contacts'));
