...
function app_init_admin_sidebar_menu_items()
{
    ...
    if (has_permission('subscriptions', '', 'view') || has_permission('subscriptions', '', 'view_own')) {
        $CI->app_menu->add_setup_menu_item('subscriptions', [ //$CI->app_menu->add_sidebar_menu_item('subscriptions', [ // تم تغييرها من اللمسة النهائية
            'name'     => _l('subscriptions'),
            'href'     => admin_url('subscriptions'),
            // 'icon'     => 'fa fa-repeat', // تم تغييرها من اللمسة النهائية
            'position' => 15,
            'badge'    => [],
        ]);
    }
    ...
    if ((!is_staff_member() && get_option('access_tickets_to_none_staff_members') == 1) || is_staff_member()) {
        ...
        $CI->app_menu->add_setup_menu_item('support', [ //$CI->app_menu->add_sidebar_menu_item('support', [ // تم تغييرها من اللمسة النهائية
        ...
        if ($enable_badge) {
            foreach ($statuses as $status) {
                $CI->app_menu->add_setup_children_item('support', [ //$CI->app_menu->add_sidebar_children_item('support', [ // تم تغييرها من اللمسة النهائية
 ...
 // تم تغييرها من اللمسة النهائية
    ...
    // Utilities
    $CI->app_menu->add_setup_menu_item('utilities', [ // $CI->app_menu->add_sidebar_menu_item('utilities', [ // تم تغييرها من اللمسة النهائية
        'collapse' => true,
        'name'     => _l('als_utilities'),
        'position' => 55,
        //'icon'     => 'fa fa-cogs', // تم تغييرها من اللمسة النهائية
        'badge'    => [],
    ]);
    ...
    if (is_admin()||is_branchadmin()) {
        ...
        $CI->app_menu->add_setup_menu_item('support', [ //$CI->app_menu->add_setup_menu_item('support', [ // تم تغييرها من اللمسة النهائية
            'collapse' => true,
            'name'     => _l('support'),
            'position' => 15,
            'badge'    => [],
