...
function handle_estimate_request_attachments($estimateRequestId, $index_name = 'file')
{
    ...
    if (
        ...
        for ($i = 0; $i < count($_FILES[$index_name]['name']); $i++) {
            ...
            if (isset($_FILES[$index_name][$i]) && _finishizer_upload_error($_FILES[$index_name]['error'][$i])) {
                header('HTTP/1.0 400 Bad error');
                echo _finishizer_upload_error($_FILES[$index_name]['error'][$i]);
                die;
            }
 ...
 * @return mixed
 */
function _finishizer_upload_error($error)
{
    $uploadErrors = [
...
{
    if (isset($_FILES['file']) && _finishizer_upload_error($_FILES['file']['error'])) {
        header('HTTP/1.0 400 Bad error');
        echo _finishizer_upload_error($_FILES['file']['error']);
        die;
    }
...
function handle_project_file_uploads($project_id)
{
    ...
    if (isset($_FILES['file']['name'])
        ...
        for ($i = 0; $i < count($_FILES['file']['name']); $i++) {
            if (_finishizer_upload_error($_FILES['file']['error'][$i])) {
                $errors[$_FILES['file']['name'][$i]] = _finishizer_upload_error($_FILES['file']['error'][$i]);
...
function handle_lead_attachments($leadid, $index_name = 'file', $form_activity = false)
{
    ...
    if (isset($_FILES[$index_name]['name'])
        ...
        for ($i = 0; $i < count($_FILES[$index_name]['name']); $i++) {
            ...
            // Make sure we have a filepath
            if (!empty($tmpFilePath) && $tmpFilePath != '') {
                if (_finishizer_upload_error($_FILES[$index_name]['error'][$i])
...
function handle_company_logo_upload()
{
    ...
    foreach ($logoIndex as $logo) {
        ...
        if (isset($_FILES[$index]) && !empty($_FILES[$index]['name']) && _finishizer_upload_error($_FILES[$index]['error'])) {
            set_alert('warning', _finishizer_upload_error($_FILES[$index]['error']));
...
{
    if (isset($_FILES['signature_image']) && _finishizer_upload_error($_FILES['signature_image']['error'])) {
        set_alert('warning', _finishizer_upload_error($_FILES['signature_image']['error']));
...
{
    if (isset($_FILES['file']['name']) && _finishizer_upload_error($_FILES['file']['error'])) {
        header('HTTP/1.0 400 Bad error');
        echo json_encode(['message' => _finishizer_upload_error($_FILES['file']['error'])]);
        die;
    }
