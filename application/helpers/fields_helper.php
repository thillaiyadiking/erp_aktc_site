...
function generate_html_checkboxes($field)
{
    $html = '';

    // Check if the field is a checkbox group
    if ($field['type'] === 'checkbox') {
        // Begin the checkbox group wrapper
        $html .= '<div class="form-group">';

        // Add the label for the checkbox group
        if (!empty($field['label_name'])) {
            $html .= '<label>' . $field['label_name'] . '</label>';
        }

        // Extract the options
        $options = explode(',', $field['options']);
        // $selected_values = isset($field['selected_values']) ? $field['selected_values'] : [];
        ...
        foreach ($options as $option) {
            ...
            if (!empty($option)) {
                ...
                $is_checked = $field['is_supplier'] == 1 ? 'checked' : '';

                // Checkbox input with a name attribute derived from the field name
                $html .= '<input type="checkbox" name="' . $field['name'] . '"  ' . $is_checked . '>';

                // Label for the checkbox
                $html .= '<label>' . _l($option) . '</label>';

                $html .= '</div>';
            }
        }

        // Close the checkbox group wrapper
        $html .= '</div>';
    }
