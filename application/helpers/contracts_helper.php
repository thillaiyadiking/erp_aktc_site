...
function count_recently_created_contracts($days = 7, $staffId = null)
{
    ...
    return total_rows(db_prefix() . 'contracts', 'dateadded BETWEEN "' . $diff1 . '" AND "' . $diff2 . '" 
                                    AND trash=0' . (count($where_own) > 0 ? ' AND' . db_prefix() . 'contracts. addedfrom=' . $staffId : ''));
}
...
function count_active_contracts($staffId = null)
{
    ...
    return total_rows(db_prefix() . 'contracts', '((DATE(dateend) >"' . date('Y-m-d') . '" AND trash=0' . (count($where_own) > 0 ? ' AND' . db_prefix() . 'contracts.addedfrom=' . $staffId : '') . ') 
...
function count_expired_contracts($staffId = null)
{
    ...
    return total_rows(db_prefix() . 'contracts', '(DATE(dateend) <"' . date('Y-m-d') . '" 
                                                    AND trash=0' . 
                                                    (count($where_own) > 0 ? ' AND' . db_prefix() . 'contracts. addedfrom=' . $staffId : '') . ')');
...
function count_trash_contracts($staffId = null)
{
    ...
    return total_rows(db_prefix() . 'contracts', 'trash=1' . (count($where_own) > 0 ? ' AND' . db_prefix() . 'contracts. addedfrom=' . $staffId : '') );
