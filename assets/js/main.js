...
function clear_item_preview_values(default_taxes) {
    ...
    var previewArea = $('.main');
    var last_unit=previewArea.find('td.custom_field select[data-fieldid="4"]').selectpicker('val'); // cf
    previewArea.find('textarea').val(''); // includes cf
    previewArea.find('td.custom_field input[type="checkbox"]').prop('checked', false); // cf
    previewArea.find('td.custom_field input:not(:checkbox):not(:hidden):not([data-fieldid="3"])').val(''); // cf // not hidden for chkbox hidden helpers
    previewArea.find('td.custom_field select').selectpicker('val', last_unit); // cf
    ...
    previewArea.find('input[name="unit"]').val(last_unit);
    ...
    get_taxes_dropdown_template(tax_name, data.taxname).done(function (tax_dropdown) {
        ...
        if (custom_fields.length > 0) {
            ...
            $.each(custom_fields, function () {
                ...
                //add quantity before supplay_price
                if($(this).find('input[data-fieldid=7]').length){
                    ...
                    table_row += '<input type="hidden" placeholder="' + app.lang.unit + '" name="newitems[' + item_key + '][unit]" class="form-control input-transparent text-right" value="' + data.unit + '">';
                ...
                if (cf_field.is(':checkbox')) {
                    ...
                    if (cf_field.is('input')) {
                        $.each(cf_field,function (index){
                            cf_name = 'newitems[' + item_key + '][custom_fields][items][' + $(this).attr('data-fieldid') + ']';
                            cf.find(`[data-fieldid=${$(this).attr('data-fieldid')}]`).attr('value', $(this).val());
                            cf.find(`[data-fieldid=${$(this).attr('data-fieldid')}]`).attr('name', cf_name);
                ...
                table_row += `<td class="${$(this).attr("class")}">${cf_html}</td>`;
            });
        }
        ...
        table_row += '<td class="taxrate hidden">' + tax_dropdown + '</td>';
        ...
        clear_item_preview_values();
        reorder_items();
        // when change value  length or width
        // console.log($(`input[name="newitems[${item_key}][custom_fields][items][1]"`));
        CreatEvents(`newitems[${item_key}][custom_fields][items]`,`newitems[${item_key}][qty]`,`newitems[${item_key}][rate]`,`newitems[${item_key}][unit]`);
...
function get_item_preview_values() {
    ...
    response.taxname = $('.main select.tax').selectpicker('val')?$('.main select.tax').selectpicker('val'):0.15;
